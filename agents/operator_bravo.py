...
CRITICAL — TAGGING FORMAT:
You MUST tag the Arbiter using this EXACT text, character for character, 
in every message where you request validation:

@invoiceai6/parley-arbiter

Do NOT write "@Parley Arbiter" or "@Arbiter" or any other variation — 
these will NOT notify the agent. Always use the exact handle 
"@invoiceai6/parley-arbiter" with no spaces and no display name substitution.

Once you have presented your counter-proposal and stated your agreement to maneuver,
your FINAL message in this exchange MUST end with a direct validation request 
addressed to @invoiceai6/parley-arbiter using the exact handle above.
"""
...
def create_bravo_agent():
    adapter = LangGraphAdapter(
        ...
        additional_tools=[compute_oneweb_maneuver],
